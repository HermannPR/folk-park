...
The next implementation stage is the `HistoryRepository` interface and system-SQLite implementation with transactional migrations, bounded search, exact composition/state recall, lineage, favorites/tags, soft deletion, retention cleanup, and database-failure isolation. Preset/history processor and UI integration remains after that repository stage.
