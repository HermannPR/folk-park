...
- Persistent audition keyboard checkpoint: `47906ac Hardened global keyboard audition`. One keyboard owner now remains mounted across every workspace; mapped A–P keydown/keyup events are consumed outside text-entry targets, key repeat remains suppressed, and pointer-played buttons blur after release. UI 19/19, Debug 18/18, Release 19/19, pluginval strictness 5, installed/build parity, and installed finite-stereo MIDI smoke pass. Real FX-workspace automation observed C3 rise to one active voice and return to zero on release. The audible macOS/FL result remains a human check.
- The producer reported intermittent composition saturation/crack/distortion on 2026-08-24 and requested a dedicated performance/optimization milestone. Treat it as release-blocking audio stability work: distinguish host deadline misses from internal clipping/non-finite/feedback/voice-pressure causes with measurements before applying a limiter or lowering gain.
- M9 Audio Stability is active on `feat/m9-audio-stability`, stacked on `feat/rhythm-lab-r1`. Its contract is `plans/M9_AUDIO_STABILITY.md`. The first implementation task is deterministic reproduction/telemetry; do not start with a limiter or unmeasured quality reduction.
- M9 telemetry checkpoint: `95f1db1 Implemented audio pressure telemetry`. The callback now publishes bounded atomic pre-master/output peaks, over-unity samples, maximum active voices, and voice steals without clocks, logging, allocation, or locks. Seed 7007 at default parameters peaks at `0.428440`, has zero over-unity/non-finite samples, reaches 16 voices, and steals 13 voices; its optimized Release render ratio was `0.571528x` at 48 kHz/512 on this Mac. The deliberate supported extreme-gain fixture peaks at `16.881600` with 17,456 over-unity samples. This proves an output-ceiling gap and normal composition voice pressure, not the exact cause of the producer's intermittent audible failure.
- M9 private draft PR #10 is stacked with base exactly `feat/rhythm-lab-r1` and head `feat/m9-audio-stability`: <https://github.com/HermannPR/folk-park/pull/10>.
- M9 render-headroom checkpoint: `d23aa94 Optimized composition render headroom`. Pre-repair normal 96 kHz composition was slower than real time (`1.14778x`–`1.16321x`); after silent-lane elision and exact pitch/frequency/mip caching, the complete 44.1/48/96 kHz × 32–1024 matrix stays at `0.119538x`–`0.262954x`. Heavy 96 kHz/64 with both oscillators at eight unison and all effects measures `0.669342x`. Debug 18/18, Release 19/19, pluginval strictness 5, installed parity/render, and current Release Standalone restart pass. Installed hash is `5377f6dcb0af792cacf0733415b972469e09978b8faa1f2b786b733517b89250`. Human listening and the separate output-ceiling repair remain pending.
- Jarvis reliability repair is verified locally. The React workspace now reconciles authoritative native A/B state before proposal creation and after any failed A/B action, so an external host edit cannot leave stale active controls or block the next request. The processor omits host-quantized no-op changes while retaining useful changes and reports an all-matching sound clearly. The walkthrough's visible `0.50` intensity is now also a committed answer. UI 19/19, Debug 18/18, Release 19/19, pluginval strictness 5, installed parity/render, and real Release Standalone sound-proposal, invalidation-recovery, and composition-candidate interactions pass. Installed VST3 hash is `a93cbf855ed56f6f7ed8010164846fedbfbc067c5ebf5efcbb936f62e6eb1253`; rollback is `~/Library/Audio/Plug-Ins/VST3/folk park.vst3.backup-20260824T162329Z`. Audible quality and FL Studio remain human-required.
