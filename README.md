...
The product-growth plan includes [Rhythm Lab](plans/RHYTHM_LAB.md). R1 now has strict versioned contracts, a sample-free synthesized kick/snare/closed-hat/open-hat/percussion engine, distinct deterministic indie/rock, Eurodance, techno, funk, and jazz profiles, explicit native candidate acceptance, channel-10 MIDI export/reopen parity, and zero measured callback allocations. Processor playback, project state, the visual workspace, and human listening remain next; the README does not present them as complete. Later Break Lab work treats Amen-style material as a licensed/original/user-import slicing workflow rather than silently bundling a copyrighted recording.

The active hardening priority is [M9 Audio Stability](plans/M9_AUDIO_STABILITY.md), created after the producer reported intermittent crackling and complete distortion during composition audition. M9 now measures cumulative pre-master/output peak, over-unity samples, maximum active voices, and voice steals through bounded atomic telemetry. It reproduced normal-composition deadline failure at 96 kHz, then eliminated silent unison-lane work and cached stable pitch/mip calculations: the full default 44.1/48/96 kHz × 32–1024 matrix now uses roughly 12%–26% of sequential real time, while a heavy 96 kHz/64 two-oscillator/eight-unison/all-effects case uses roughly 67%. A deliberate supported extreme-gain fixture still reaches `16.8816`, so output-ceiling work and producer listening remain open rather than being presented as solved.
